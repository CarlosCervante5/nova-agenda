...
export default async function LinksPage({ params }: { params: { clientSlug: string } }) {
  ...
  if (!client) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: '#0f0f1a' }}>
        <p style={{ color: '#888' }}>Negocio no encontrado.</p>
      </div>
    );
  }
  ...
  const links: { title: string; url: string; icon: string; color?: string }[] = [];

  if (client.instagram) {
    links.push({
      title: 'Instagram',
      url: client.instagram.startsWith('http') ? client.instagram : `https://instagram.com/${client.instagram.replace('@', '')}`,
  ...
  if (client.facebook) {
    links.push({
      title: 'Facebook',
      url: client.facebook.startsWith('http') ? client.facebook : `https://facebook.com/${client.facebook}`,
  ...
  if (client.whatsappPhone) {
    const phone = client.whatsappPhone.replace(/[^0-9]/g, '');
    links.push({
      title: 'WhatsApp',
  ...
  links.push({
    ...
    color: client.primaryColor || '#2dd4bf',
  });

  links.push({
    title: 'Programa de fidelidad',
    url: `/${client.slug}?loyalty=1`,
  ...
  return <LinktreeClient client={client} links={links} />;
}
